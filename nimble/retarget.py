...
from common.cpu import configure_compute_threads, resolve_k8s_shard, shard_ids
...
ACTIVATION_METHODS = ("none", "static_optimization", "moco_track")
...
def load_subject_trials(ppid, base_directory):
    ...
    c3d_folder = os.path.join(
        get_kinematic_directory(base_directory), str(ppid), "Generated_C3D_files"
    )
...
def load_subject_info(base_directory):
    ...
    kinematic_directory = get_kinematic_directory(base_directory)
    ...
    participants_info_dir = get_participants_info_directory(base_directory)

    # Load metadata
    subject_log_path = os.path.join(participants_info_dir, "Subject Log.xlsx")
    metadata = {}
    ...
    sampling_freq_path = os.path.join(participants_info_dir, "Sampling Frequency.xlsx")
    freq = {}
...
def process_subject(args):
    ...
    output_b3d_path = os.path.join(output_dir, f"{ppid}.b3d")
    if skip_existing and os.path.isfile(output_b3d_path):
        logging.info("Skipping existing %s", output_b3d_path)
        return ppid, {}
    ...
    for idx, trial in enumerate(trials):
        ...
        if activation_method != "none":
            try:
                ...
                from nimble.muscle_b3d import pack_muscle_activations

                configure_opensim_logging(opensim_log_level)
                q_traj = np.ascontiguousarray(poses.T, dtype=np.float64)
                act_cfg = MuscleActivationConfig(
                    activation_method=activation_method,
                    fps=float(sampling_frequency),
                    mass_kg=float(mass) if mass else 70.0,
                    opensim_log_level=opensim_log_level,
                )
                act_result = compute_muscle_activation(q_traj, cfg=act_cfg)
                b3d_subject.setCustomValueNames(["muscle_activations"])
                b3d_trial.setCustomValues(pack_muscle_activations(act_result.activations))
            except Exception as exc:
                logging.warning(
                    "Muscle activation (%s) failed for subject %s trial %s: %s",
                    activation_method,
                    ppid,
                    trial["name"],
                    exc,
...
def _manifest_path(output_dir: str, shard_index: int, num_shards: int) -> Path:
    root = Path(output_dir)
    if int(num_shards) > 1:
        return root / f"retarget_manifest.{int(shard_index):04d}.jsonl"
    return root / "retarget_manifest.jsonl"


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--dataset_path", type=str, default=ATHLETES_183_RAW_DIR, help="Path to the dataset")
    parser.add_argument("--output_dir", type=str, default=ATHLETES_183_RETARGETED_DIR, help="Directory to save output .b3d files")
    ...
    parser.add_argument("--num_shards", type=int, default=None, help="Total shard count for distributed retargeting")
    parser.add_argument("--shard_index", type=int, default=None, help="This worker shard index (0-based)")
    parser.add_argument(
        "--activation_method",
        type=str,
        default="none",
        choices=["none", "static_optimization", "moco_track"],
        help="Muscle activation estimation method (none = kinematics only)",
    )
    parser.add_argument("--skip_existing", action="store_true", help="Skip subjects whose output .b3d already exists")
    parser.add_argument("--opensim_log_level", type=str, default="Off", help="OpenSim log level during muscle solves")

    args = parser.parse_args()
    activation_method = str(args.activation_method).strip()
    if activation_method not in ACTIVATION_METHODS:
        raise ValueError(f"activation_method must be one of {ACTIVATION_METHODS}, got {activation_method!r}")
    ...
    selected_ppids = shard_ids(selected_ppids, shard_index, num_shards)
    if num_shards > 1:
        logging.info(
            "Shard %d/%d: %d subject(s) assigned",
            shard_index + 1,
            num_shards,
            len(selected_ppids),
        )
    ...
    for ppid, trial_errors_by_name in results:
        processed_ppids.append(ppid)
    ...
    manifest_path = _manifest_path(args.output_dir, shard_index, num_shards)
    with open(manifest_path, "w", encoding="utf-8") as manifest_file:
        for ppid in processed_ppids:
            manifest_file.write(
                json.dumps(
                    {
                        "subject_id": ppid,
    ...
    logging.info("Wrote manifest %s (%d subjects)", manifest_path, len(processed_ppids))
